...
function GatekeeperPage() {
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await fetch(`https://visitor-pass-1.onrender.com/api/visitors?${queryParams}`);
      if (!response.ok) {
  ...
  const handleUpdateExitTime = async (id) => {
    try {
      ...
      const response = await fetch(`https://visitor-pass-1.onrender.com/api/visitors/${id}`, {
        method: 'PUT',
